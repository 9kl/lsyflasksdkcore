...
from lsyflasksdkcore.linq.linq_exceptions import NoSuchElementError
...
class LinqQuery(Iterable[T]):
    ...
    def order(
        self, key: Callable[[T], Any] = lambda x: x, descending=False
    ) -> LinqQuery[T]:
    ...
    def union(
        self, outer: Iterable[T], key: Callable[[T], Any] = lambda x: x
    ) -> LinqQuery[T]:
